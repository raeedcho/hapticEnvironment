...
void initHaptics(void)
{
  ...
  hapticsData.tool = new cToolCursor(controlData.world);
  hapticsData.tool->m_hapticPoint->m_sphereProxy->m_material->setRed();
  controlData.world->addChild(hapticsData.tool);
  ...
  cout << "Maximum device force: " << hapticsData.maxForce << endl;
  cout << "Maximum device stiffness: " << hapticsData.hapticDeviceInfo.m_maxLinearStiffness << endl;
  cout << "Maximum device damping: " << hapticsData.hapticDeviceInfo.m_maxLinearDamping << endl;
}
